...
public class CuentaContableRESTService extends RESTServiceTemplate<CuentaContableDomain> implements CuentaContableUseCase {
    ...
    private final CuentaContableUseCase cuentaContableUC = A_ModuleGestionContabilidadRESTConfig.cuentaContableUC;
    ...
    public List<CuentaContableDomain> findAllCuenta(@PathVariable(TIPO_CUENTA) Integer idTipoCuenta) throws RuntimeException {
        return cuentaContableUC.findAllCuenta(idTipoCuenta);
    }

    @Override
    @GetMapping(CUENTA_CONTABLE_FIND_ALL_SEARCH_PATH)
    public List<CuentaContableDomain> findAll(@PathVariable(SEARCH_TEXT) String searchText) throws RuntimeException {
